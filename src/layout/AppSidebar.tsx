...
const AppSidebar: React.FC = () => {
  const { isExpanded, isMobileOpen, isHovered, setIsHovered, session } = useSidebar();
  const pathname = usePathname();
  const role = session?.role || "USER";

  // Filter navItems sesuai role
  const filteredNavItems = navItems.filter(item => roleAccess[role].allowed.includes(item.name));

  // Jika user belum approved, tampilkan sidebar kosong atau menu khusus
  ...
  // Remove filteredNavItems completely
  ...
  return (
    ...
    >
      <div
        className={`py-6 ml-2 mb-5 flex  ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"
          }`}
      >
        <Link href="/">
          {isExpanded || isHovered || isMobileOpen ? (
